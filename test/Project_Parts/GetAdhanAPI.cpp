...
void screenDisplay(void *parameters){
  ...
  for(;;){
    ...
    if(screenStatusCfx.screenCurrentIndex == 0){
      ...
      screen.sendBuffer();
    } else if(screenStatusCfx.screenCurrentIndex == 4){
      // NEW ADHAN SCREEN
...
void setup(){
  ...
  screenAdhanQueue_handle = xQueueCreate(SCREEN_ADHAN_QUEUE_SIZE, sizeof(AdhanTimings)); // ADD THIS LINE
